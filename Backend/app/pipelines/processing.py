...
def process_after_edit(recording_id: str, edited_text: str) -> Dict[str, Any]:
    ...
    - Segment edited transcript (based on sentences in edited text)
    - Detect PII on edited text
    - Save redacted edited transcript
    - Compute prosody using timed sentences from metadata (WhisperX timings)
    - Update metadata (segments + pii + prosody)
    """
    import os
    ...
    for seg in segments:
        ...
        for sid in sids:
            try:
                sentence_to_segment[int(sid)] = int(seg.id)
            except Exception:
                pass
    ...
    for rel in old_seg_paths:
        if not rel or rel == new_segments_path:
            continue
    ...
    try:
        ...
        if audio_rel and timed_sentences:
            audio_abs = store.abs_path(audio_rel)
            audio_array = transcriber._load_audio_ffmpeg(
                audio_abs, sr=getattr(transcriber, "sample_rate", 16000)
            )
            ...
            for s in timed_sentences:
                ...
                if sid is None or st is None or en is None:
                    continue
            ...
            if sentence_objs:
                ...
                for pf in prosody_features or []:
                    ...
                    if sid is None:
                        continue
                    pf.segment_id = sentence_to_segment.get(int(sid))
    except Exception:
        prosody_features = []
