...
/**
 * Build the tour steps. When the library is still empty (typically the very
 * first run) the source-dependent steps switch to invitational, future-tense
 * copy so the tour never tells you to "tick this source" when there's nothing
 * to tick yet. Pass hasSources=false for that empty state.
 */
export function buildTourSteps(hasSources = true): TourStep[] {
  return [
    ...
    hasSources
      ...
      : { target: "include", eyebrow: "Context", title: "Choose what the AI reads.", body: "Every source gets an include checkbox. Only the ones you tick are used when you chat or search, so you control exactly what Reflect reasons over." },
    ...
    { target: null, eyebrow: "You're ready", title: "Start with one source.", body: "Add a voice note or a few lines about your week, and Reflect takes it from there. You can replay this tour anytime from Settings." },
  ];
}

export const TOUR_STEPS: TourStep[] = buildTourSteps(true);
