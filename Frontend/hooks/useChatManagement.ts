...
export function useChatManagement({ rawSources, setRawSources, setProcessingSources }: UseChatManagementOptions) {
  ...
  const persistMessage = async (
    chatId: number,
    payload: Parameters<typeof api.appendChatMessage>[1],
    // Fields merged onto the message only in local state, never sent to the backend —
    // used to attach a reflection reply's citation source units (session-scoped only,
    // see streamFacilitator's onDone; lost on reload since they're not persisted).
    clientOnlyFields?: Partial<ChatMessageRecord>
  ) => {
    try {
      const created = await api.appendChatMessage(chatId, payload)
      const merged = clientOnlyFields ? { ...created, ...clientOnlyFields } : created
      ...
      if (activeChatIdRef.current === chatId) setActiveChatMessages((prev) => [...prev, merged])
      void refreshChats()
      return merged
    } catch (error) {
    ...
    try {
      ...
      if (asReflection) {
        ...
        void api.streamGeneratedQuestion(
          ...
          {
            onProgress: () => {},
            // Still no facilitator reply — "one answer per question" is unchanged. This
            // only tells the user their answer actually landed server-side (Gist/Open
            // Thread absorbed it), as distinct from nothing having happened at all.
            onDone: () => {
              if (activeChatIdRef.current === chatId) toast.success("Got it")
            },
    ...
    await new Promise<void>((resolve) => {
      api.streamGeneratedQuestion(
        ...
        },
        {
          ...
          onDone: async ({ text, model, fallbackKind, units }) => {
            try {
              if (fallbackKind) {
                ...
                // Retrieved units ride along as client-only `sources` so chat-messages.tsx
                // can resolve the reply's {{source_id:unit_id}} citation tokens — not
                // persisted server-side, so this only works for the live session.
                await persistMessage(
                  chatId,
                  { role: "question", text, model, gibbs_step: step },
                  units && units.length
                    ? { sources: units.map((u) => ({ source_id: u.source_id, chunk_id: u.unit_id, text: u.text })) }
                    : undefined
                )
              }
            } finally {
  ...
  // The "Next question" lever (formerly two separate levers: the silent "Answer" —
  // persist typed text, no visible reply — and the standalone "Ask another question"
  // pill — re-ask, no persist). Merged per design change: record whatever's in the box
  // as this stage's answer (if any), then always ask a full follow-up question at the
  // SAME stage with a real visible reply. Works with or without typed text, same as the
  // old pill did. Mirrors continueStage's shape exactly, just without advancing the stage.
  const askNextQuestion = async () => {
    if (gibbsGenerating) return
    if (inputValue.trim()) {
      const created = await submitText("reflect")
      if (!created) return
      await generateGibbsQuestion(gibbsStep, "clarifying", [created])
    } else {
  ...
  return {
    ...
    startReflection, beginReflection, advanceGibbsStep, exitReflection, handleSelectGibbsStep, beginNewCycle,
  }
}
