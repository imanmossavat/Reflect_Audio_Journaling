...
export default function SourceDetailPage() {
    ...
    const handleSaveDate = async (isoValue: string) => {
        ...
        try {
            const updated = await api.patchSource(source.id, { created_at: new Date(isoValue).toISOString() })
            setSource(updated)
        } catch (err) {
                                            ...
                                            <div className="rounded-lg border bg-muted/20 p-3">
                                                ...
                                                {editingDate ? (
                                                    <input
                                                        type="datetime-local"
                                                        autoFocus
                                                        defaultValue={new Date(source.created_at).toISOString().slice(0, 16)}
                                                    ...
                                                    <div className="mt-1 flex items-center gap-2 text-sm font-medium">
                                                        ...
                                                        {new Date(source.created_at).toLocaleString('en-US', { dateStyle: "medium", timeStyle: "short", hour12: false })}
                                                    </div>
                                                )}
                                            </div>
                                            <div className="rounded-lg border bg-muted/20 p-3">
