...
import EditableTitle from "@/components/TitleEditor";
import TranscriptEditor from "@/components/TranscriptEditor";
import ProsodySummary from "@/components/ProsodySummary";
...
export default function RecordingClient({ api, id, initialData }: { api: string; id: string; initialData: RecordingData }) {
  ...
  // Safety check: If backend returned 404 or data is corrupted
  ...
  // PRIMARY LIST: Always use pii_edited for interaction logic
  ...
  const refreshMeta = React.useCallback(async () => {
    try {
      ...
      if (r.status === 404) {
        setData({} as any); // Trigger not found view
        return;
      }
      const d = (await r.json()) as RecordingData;
      setData(d);
    } catch (err) {
  ...
  const syncPii = React.useCallback(async (newPii: PiiHit[]) => {
    const res = await fetch(`${api}/api/recordings/${id}/pii`, {
      ...
      body: JSON.stringify({ findings: newPii }),
    });
  ...
  const onAddPii = (newHit: PiiHit) => {
    // Add recording_id if missing from the component scope
  ...
  const handleConfirmObfuscation = async () => {
    ...
    // Shift coordinates for all subsequent PII hits
    ...
    try {
      // 1. Save the new text and shifted PII
      await saveTranscriptText(updatedText);
      await syncPii(nextPii);

      // 2. Finalize (re-segment and re-redact)
      const form = new FormData();
      form.append("recording_id", id);
      form.append("edited_transcript", updatedText);
  ...
  return (
      <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950">
        <div className="mx-auto p-6 space-y-3">
          <Link href="/recordings" className="text-sm text-zinc-600 hover:underline dark:text-zinc-400">← Back to library</Link>
              ...
              <Card className="shadow-sm border-zinc-200 dark:border-zinc-800">
                ...
                <CardContent className="space-y-4 text-sm">
                  ...
                  <div className="pt-2">
                    ...
                    <TagsEditor api={api} id={id} tags={tags} onSaved={refreshMeta} />
                  </div>
                </CardContent>
              </Card>
            </div>
